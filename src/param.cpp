...
void processCmd(){
    ...
    if (cmdBuffer[0] == '?'){ // when help is requested we print the instruction
        ...
        printf(" S(Sport Frsky), F(Fbus Frsky), C(CRSF/ELRS), H(Hott), M(Mpx), 2(Sbus2 Futaba), J(Jeti)\n");
        printf("                                                   E(jeti Exbus), L (spektrum SRXL2), I(IBus/Flysky), R(RLink/RadioLink), X(Spektrum Xbus) or T(Hitec)\n");
    ...
    // change PRI pin
    if (( strcmp("PRI", pkey) == 0 ) && (config.protocol != 'R')){ 
    ...
    // change TLM pin
    if (( strcmp("TLM", pkey) == 0 ) && (config.protocol != 'R')){ 
        ui = strtoul(pvalue, &ptr, 10);
        if ( *ptr != 0x0){
            printf("Error : pin must be an unsigned integer\n");
...
// force dummy negative value
    ...
    if ( strcmp("PROTOCOL", pkey) == 0 ) { // if the key is BAUD
        if (strcmp("S", pvalue) == 0) {
            ...
            printf("Error : protocol must be S(Sport Frsky), F(Fbus Frsky), C(CRSF=ELRS), J(Jeti), E(jeti Exbus), H(Hott), M(Mpx), 2(Sbus2 Futaba), L(SRXL2 Spektrum), I(Ibus/Flysky), R(Rlink/RadioLink), X(Spektrum Xbus) or T(Hitec)\n");
        }
    }
...
void setupConfig(){   // The config is uploaded at power on
    if (*flash_target_contents == CONFIG_VERSION ) {
        ...
        config.CamPitchChannel = 255;
        config.CamRollChannel = 255;
        config.CamPitchRatio = 255;
        config.CamRollRatio = 255;
    }
